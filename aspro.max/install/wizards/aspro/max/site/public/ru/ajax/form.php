...
<?elseif($form_id === 'TABLES_SIZE'):?>
	...
	$requestedFile = isset($request['url']) && $request['url'] ? $_SERVER['DOCUMENT_ROOT'].$request['url'] : '';
	if (
		$requestedFile
		&& strpos(realpath($requestedFile), realpath($_SERVER['DOCUMENT_ROOT'].SITE_DIR.'include')) === 0
		&& file_exists($requestedFile)
	) {
		$dirname = dirname($requestedFile);
		$basename = basename($requestedFile);

		$arFiles = glob($dirname.'/*.php');
