...
if (CModule::IncludeModule(ASPRO_MODULE_NAME) && $bInstallExtModule) {
	try {
		$result = Thematics::unzip([
			'templateID' => $extModuleShortId,
			'moduleID' => $extModuleId,
		]);

		$unZipFile = $result['unZipFile'];
	}
	catch (\Exception $e) {
...
if (strlen($errorMessage)) {
	...
	if ($unZipFile) {
		...
		$status = GetMessage(
			'EXT_SERVICE_PREPARE_DATA_UNZIP_PART',
			array_merge(
				['#EXT_MODULE_ID#' => $extModuleId],
				$result['arStatus']
			)
		);
		...
		$status = GetMessage('EXT_SERVICE_PREPARE_DATA_COPY', ['#EXT_MODULE_ID#' => $extModuleId]);
