...
if (!isset($extModuleClass)) return;

ob_start();
$errorMessage = '';

$bInstallExtModule = $installExtModule === "Y" && !CModule::IncludeModule($extModuleId);
unset($_SESSION['aspro_ext_'.$extModuleId]);

if (CModule::IncludeModule(ASPRO_MODULE_NAME) && $bInstallExtModule) {
	// check module index file
	try {
		if (@file_exists($_SERVER['DOCUMENT_ROOT'].'/bitrix/modules/'.$extModuleId.'/install/index.php')) {
			include_once($_SERVER['DOCUMENT_ROOT'].'/bitrix/modules/'.$extModuleId.'/install/index.php');

			$obModule = new $extModuleClass;
			if (!$obModule->IsInstalled()) {
				$obModule->DoInstall();
				$_SESSION['aspro_ext_'.$extModuleId] = true;
			}
		}
		else {
			throw new \Exception(GetMessage('EXT_SERVICE_PREPARE_DATA_SETUP_ERROR', ['#EXT_MODULE_ID#' => $extModuleId]));
		}
	}
	catch (\Exception $e) {
...
if (strlen($errorMessage)) {
	...
	$status = GetMessage('EXT_SERVICE_PREPARE_DATA_CUSTOM', ['#EXT_MODULE_ID#' => $extModuleId]);
