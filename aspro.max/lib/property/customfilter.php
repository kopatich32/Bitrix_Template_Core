...
class CustomFilter{
	...
	static function getJSFile() {
		return '/bitrix/js/aspro.max/property/customfilter_control.js';
	}

	static function getAjaxFile() {
		return '/bitrix/tools/aspro.max/customfilter_ajax.php';
	}
	...
	static function GetPropertyFieldHtml($arProperty, $value, $strHTMLControlName){
		...
		if($bDetailPage){
			if(!isset($cache)){
				$cache = array();
				$jsFile = static::getJSFile();
				$ajaxFile = static::getAjaxFile();
